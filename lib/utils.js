...
async function endSimulatorDaemons () {
  ...
  try {
    ...
    logger.warn('Could not stop simulator daemons, carrying on anyway!');
  }
  try {
    ...
    removeCmd = 'launchctl list | grep com.apple.CoreSimulator | cut -f 3 | xargs -n 1 launchctl remove';
    await exec('bash', ['-c', removeCmd]);
  } catch (err) {
    logger.warn('Could not remove simulator daemons, carrying on anyway!');
  }
...
export default { rootDir, removeInstrumentsSocket, getAndCheckXcodeVersion, prepareIosCaps,
  ...
  getDeviceString, endSimulatorDaemons, getSimForDeviceString };
