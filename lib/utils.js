...
async function parseLocalizableStrings (opts) {
  ...
  let lprojRoot = path.resolve(app, `${language}.lproj`);
  if (!await fs.exists(lprojRoot)) {
    lprojRoot = path.resolve(app, localizableStringsDir || '');
    logger.debug(`No '${path.resolve(app, `${language}.lproj`)}' folder has been found. ` +
      `Getting the strings from '${lprojRoot}'`);
  }
