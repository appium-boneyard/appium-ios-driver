...
class Instruments {
  ...
  constructor (opts) {
    ...
    // config
    const props = [
      'app', 'termTimeout', 'flakeyRetries', 'udid', 'bootstrap',
  ...
  async configure () {
    ...
    if (this.xcodeVersion.major < 5 && this.withoutDelay) {
      log.info('instruments-without-delay for Xcode 4 has been removed ' +
               'because of macOS notarization. ' +
               'You can disable instruments-without-delay ' +
  ...
  async launchOnce () {
    ...
    this.proc.stderr.pipe(through(function onStderr (output) {
      actOnStderr(output);
      this.queue(output);
    })).pipe(errorStream())
  ...
  async spawnInstruments () {
    ...
    instrumentsExecArgs = _.map(instrumentsExecArgs, function instrumentArgs (arg) {
      if (arg === null) {
