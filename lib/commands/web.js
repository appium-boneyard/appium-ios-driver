...
extensions.getExtraNativeWebTapOffset = async function () { // eslint-disable-line require-await
...
helpers.convertElementsForAtoms = function (args = []) {
  let newArgs = [];
  for (let arg of args) {
    let hasElementKey = false;
    for (let elementKey of ['ELEMENT', util.W3C_WEB_ELEMENT_IDENTIFIER]) {
      if (util.hasValue(arg) && util.hasValue(arg[elementKey])) {
        hasElementKey = true;
        let atomsElement = this.getAtomsElement(arg[elementKey]);
        if (atomsElement === null) {
          throw new errors.UnknownError(`Error converting element ID for using in WD atoms: '${arg[elementKey]}'`);
        }
      }
    }
