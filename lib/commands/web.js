...
import cookieUtils from 'appium-cookies';
...
commands.getCssProperty = async function (propertyName, elementId) {
  let atomsElement = this.useAtomsElement(elementId);
  return await this.executeAtom('get_value_of_css_property', [atomsElement, propertyName]);
};

commands.submit = async function (elementId) {
  if (this.isWebContext()) {
    let atomsElement = this.useAtomsElement(elementId);
...
commands.getCookies = async function () {
  ...
  let cookies = [];
  try {
    for (let [name, value] of _.pairs(cookieUtils.createJWPCookie(undefined, jsCookies))) {
...
commands.deleteCookie = async function (cookieName) {
  ...
  if (_.indexOf(_.pluck(cookies, 'name'), cookieName) === -1) {
...
extensions.webFlickElement = async function (elementId, xoffset, yoffset) {
  let atomsElement = await this.useAtomsElement(elementId);
...
extensions.nativeWebTap = async function (elementId) {
  let atomsElement = this.useAtomsElement(elementId);
  let {x, y} = await this.executeAtom('get_top_left_coordinates', [atomsElement]);
  let {width, height} = this.executeAtom('get_size', [atomsElement]);
  x = x + (width / 2);
  y = y + (height / 2);
...
extensions.translateWebCoords = async function (coords) {
  ...
  let yOffset = this.curOrientation === 'LANDSCAPE' ? this.landscapeWebCoordsOffset : 0;
...
helpers.executeAtom = async function (atom, args, alwaysDefaultFrame = false) {
  ...
  // })
  // .catch((err) => {
  //   // end things if there is an error
  ...
  //   if (done) break;
  //   await B.delay(5000);
  //   // we are waiting 5s, which is a long time,
  //   // so see if we should break again
  //   if (done) break;
