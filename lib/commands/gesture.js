...
commands.parseTouch = async function parseTouch (gestures) {
  ...
  const needsPoint = (action) => _.includes(['press', 'moveTo', 'tap', 'longPress'], action);
...
const mergeStates = function mergeStates (states) {
  ...
  const timeSequence = function timeSequence (states) {
    let seq = [];
    _.each(states, function eachState (state) {
  ...
  let merged = [];
  _.each(timeSequence(states), function eachSequence (time, index) {
    let slice = getSlice(states, index);
    let obj = {
      time,
      touch: []
    };
    _.each(slice, function eachSlice (action) {
...
commands.flick = async function flick (el, xspeed, yspeed, xoffset, yoffset, speed) {
  el = unwrapEl(el);
  if (_.every([el, xoffset, yoffset, speed], (p) => !_.isNull(p) && !_.isUndefined(p))) {
    await this.flickElement(el, xoffset, yoffset, speed);
  } else if (_.every([xspeed, yspeed], (p) => !_.isNull(p) && !_.isUndefined(p))) {
