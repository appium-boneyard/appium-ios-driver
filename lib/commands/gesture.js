...
commands.click = async function (elementId) {
  if (this.isWebContext()) {
    await this.webClick(elementId);
  } else {
    if (this.useRobot) {
      /* TODO */throw new errors.NotYetImplementedError();
    } else {
      await this.nativeTap(elementId);
    }
  }
};
...
helpers.clickCoords = async function (coords) {
  if (this.useRobot) {
    ...
    opts.duration = 0.3;
    opts.touchCount = 1;
    var command = `au.complexTap(${JSON.stringify(opts)})`;
    await this.uiAutoClient.sendCommand(command);
  }
};
...
commands.moveTo = async function (element, xoffset, yoffset) {
  if (this.isWebContext()) {
    /* TODO */ throw new errors.NotYetImplementedError();
  } else {
    xoffset = xoffset || 0;
    ...
    if(_.isNull(element) || _.isUndefined(element)) {
      ...
      let elPos = await this.getLocation(element);
      this.curCoords = {
        x: elPos.x + xoffset,
        y: elPos.y + yoffset
      };
