...
commands.mobileRotation = async function (x, y, radius, rotation, touchCount, duration, el) {
  ...
  if (el) {
    ...
    await this.uiAutoClient.sendCommand(
      `au.getElement('${el}').rotateWithOptions(${JSON.stringify(location)},${JSON.stringify(options)})`);
  } else {
    await this.uiAutoClient.sendCommand(
...
commands.flick = async function (el, xspeed, yspeed, xoffset, yoffset, speed) {
  el = unwrapEl(el);
  if (_.all([el,xoffset, yoffset, speed], (p) => {return !_.isNull(p) && !_.isUndefined(p); })) {
...
commands.moveTo = async function (el, xoffset, yoffset) {
  el = unwrapEl(el);
  if (this.isWebContext()) {
    /* TODO */ throw new errors.NotYetImplementedError();
  } else {
