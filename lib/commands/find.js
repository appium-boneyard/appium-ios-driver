...
helpers.findUIElementOrElements = async function  (strategy, selector, mult, context) {
  ...
  if (strategy === "xpath" && selector === "//*[@scrollable=true]") {
    return await this.findScrollableElOrEls(mult, context);
  }

  if (strategy === "xpath" && selector === "//*[@onScreen=true]" && context) {
...
commands.findScrollableElOrEls = async function (mult, context) {
  const scrollTypes = ['UIAScrollView', 'UIATableView', 'UIACollectionView', 'UIAWebView'];
  let command;
  let res;
  let ext = mult ? 's' : '';
  for (let i = 0; i < scrollTypes.length; i++) {
    command = `au.getElement${ext}ByType('${scrollTypes[i]}'${context})`;
    if (mult) {
      if (!res) {
        res = [];
      }
