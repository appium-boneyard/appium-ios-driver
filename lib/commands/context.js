...
extensions.listWebFrames = async function () {
  ...
  let pageArray;
  if (this.remote !== null && this.caps.bundleId !== null) {
    if (this.isRealDevice()) {
      pageArray = await this.remote.pageArrayFromJson();
    } else {
      pageArray = await this.remote.selectApp(this.caps.webviewConnectRetries);
    }
  } else {
    if (this.isRealDevice()) {
...
extensions.navToInitialWebview = async function () {
  ...
  } else if (parseInt(this.iosSdkVersion, 10) >= 7 && !this.isRealDevice() && this.caps.safari) {
...
extensions.navToViewWithTitle = async function (titleRegex) {
  ...
  let spinHandles = async () => {
    ...
    if (this.isRealDevice() || !this.caps.safari) {
