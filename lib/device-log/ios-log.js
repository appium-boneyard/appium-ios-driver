...
class IOSLog {
  constructor (opts) {
    ...
    this.logs = [];
    this.logsSize = this.objectSize(this.logs);
    this.logRow = '';
  ...
  appendLog (logObj) {
    const totalAvailHeapSize = v8.getHeapStatistics().total_available_size;
    const logObjSize = this.objectSize(logObj);
    if (logObjSize + this.logsSize >= totalAvailHeapSize / 2) {
