...
class IOSLog {
  ...
  async startCaptureRealDevice () {
    ...
    this.proc = IOSLog._getIDeviceSysLogProcess(cmd, args, env, this.udid);
  ...
  async stopCapture () {
    ...
    this.proc = null;
    IOSLog.cachedIDeviceSysLogs[this.udid] = null;
  }
   ...
   */
  static async _getIDeviceSysLogProcess (cmd, args, env, udid) {
    if (!IOSLog.cachedIDeviceSysLogs[cmd]) {
      IOSLog.cachedIDeviceSysLogs[cmd] = {};
    }
    if (!IOSLog.cachedIDeviceSysLogs[cmd][udid]) {
      IOSLog.cachedIDeviceSysLogs[cmd][udid] = new SubProcess(cmd, args, {env});
    }
    return IOSLog.cachedIDeviceSysLogs[cmd][udid];
  }
}
