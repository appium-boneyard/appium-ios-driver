...
class IOSLog {
   ...
   */
  static _getIDeviceSysLogProcess (cmd, args, env, udid) {
    if (!IOSLog.cachedIDeviceSysLogs[udid]) {
      let proc = IOSLog.cachedIDeviceSysLogs[udid] = new SubProcess(cmd, args, {env});
      proc.on('exit', () => {
        delete IOSLog.cachedIDeviceSysLogs[udid];
      });
    }

    return IOSLog.cachedIDeviceSysLogs[udid];
  }
}
