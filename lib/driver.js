 ...
 async startSession () {
  ...
  setLocServicesPrefs () {
    ...
    if (typeof this.capabilities.locationServicesAuthorized !== "undefined") {
      ...
      settings.updateLocationSettings(this.sim, this.caps.bundleId, locAuth);
    }
  }
  ...
  async startLogCapture () {
    ...
    this.logs.syslog = new IOSLog({
      udid: this.iosSimUdid || this.caps.udid
    , showLogs: this.caps.showSimulatorLog || this.caps.showIOSLog
    });
    try {
  ...
  async waitForAppLaunched  () {
    ...
    await utils.waitForCondition(10000, condFn, 500);
  }
  ...
  async startInstruments () {
    ...
    let uiAutoClientPromise = this.uiAutoClient.start();
    await this.instruments.launch();
    await uiAutoClientPromise;
  }
