...
class IosDriver extends BaseDriver {
 ...
 async startSession () {
  ...
  async startSimulator () {
    ...
    // removed moveBuiltInApp, not needed in recent iOS
    ...
    // TODO: await B.fromNode(this.startInstruments.bind(this));
    await this.onInstrumentsLaunch();
    //TODO await this.configureBootstrap();
  ...
  async instantLaunchAndQuit (needSafariDirs) {
    ...
    let condition = function () {
      ...
      let locServicesExist;
      // TODO locServicesExist = settings.locServicesDirsExist(this.sim);
  ...
  setLocServicesPrefs () {
    if (typeof this.capabilities.locationServicesEnabled !== "undefined" ||
      ...
      //TODO settings.updateSettings(this.sim, 'locationServices', {
  ...
  async onInstrumentsLaunch () {
    ...
    // TODO this.instruments.setDebug(true);
  ...
  async makeInstruments () {
    ...
    let instruments = new Instruments({
      ...
      tmpDir: path.resolve(this.caps.tmpDir , 'appium-instruments'),
