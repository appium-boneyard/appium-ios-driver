...
class IosDriver extends BaseDriver {
  ...
  async startSimulator () {
...

    {
      ...
      let dString = this.adjustedDeviceName;
      if (this.caps.app) {
  ...
  validateDesiredCaps (caps) {
    ...
    if ((caps.browserName && caps.browserName.toLowerCase() !== 'safari') &&
  ...
  async makeInstruments () {
    ...
    let instruments = new Instruments({
      ...
      simulatorSdkAndDevice: this.iosSdkVersion >= 7.1 ? this.adjustedDeviceName : null,
  ...
  get adjustedDeviceName () {
    if (this._adjustedDeviceName) {
      return this._adjustedDeviceName;
    }

    // no adjusted device name yet, so calculate it
    this.adjustedDeviceName = utils.getDeviceString(this.xcodeVersion, this.iosSdkVersion, this.opts);
    return this._adjustedDeviceName;
  }

  set adjustedDeviceName (name) {
    this._adjustedDeviceName = name;
  }
}
