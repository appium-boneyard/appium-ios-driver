...
class IosDriver extends BaseDriver {
  ...
  async createSession (caps) {
    ...
    utils.prepareIosCaps(this.opts);
  ...
  async stop() {
    this.ready = false;
    await B.all([
      () => { if (this.uiAutoClient) return this.uiAutoClient.shutdown(); }(),
      () => { if (this.instruments) return this.instruments.shutdown(); }
    ]);
    this.uiAutoClient = null;
    this.instruments = null;
  ...
  async deleteSession () {
    ...
    await this.stop();
    await this.runSimReset(); // TODO: do we need to move this to stop

    await super.deleteSession();
  }

  async executeCommand (cmd, ...args) {
    if(this.ready || _.includes(['launchApp'], cmd)) {
  ...
  async configureApp () {
    try {
      ...
      if (this.opts.app && this.opts.app.toLowerCase() === "settings") {
        ...
        logger.error(err);
        throw new Error(
          `Bad app: ${this.args.app}. App paths need to be absolute, or relative to the appium ` +
  ...
  async startSimulator () {
...

    {
      ...
      let dString = utils.getDeviceString(this.xcodeVersion, this.iosSdkVersion, this.opts);
  ...
  setLocServicesPrefs () {
    ...
    if (typeof this.opts.locationServicesAuthorized !== "undefined") {
      ...
      this.sim.updateLocationSettings(this.opts.bundleId, locAuth);
    }
  }
  ...
  async makeInstruments () {
    ...
    let bootstrapPath = await prepareBootstrap({
      ...
      autoAcceptAlerts: !(!this.opts.autoAcceptAlerts || this.opts.autoAcceptAlerts === 'false'),
      autoDismissAlerts: !(!this.opts.autoDismissAlerts || this.opts.autoDismissAlerts === 'false'),
    ...
    let instruments = new Instruments({
      ...
      simulatorSdkAndDevice: this.iosSdkVersion >= 7.1 ?
        utils.getDeviceString(this.xcodeVersion, this.iosSdkVersion, this.opts) : null,
