...
import { getSimulator, killAllSimulators, getDeviceString } from 'appium-ios-simulator';
...
class IosDriver extends BaseDriver {
  ...
  async createSim (deviceName, deviceType, sdkVersion) {
    await this.sim.create(deviceName, deviceType, sdkVersion);
  }
  ...
  async runSimReset () {
    if (this.opts.reset || this.opts.fullReset) {
      ...
      if (this.opts.fullReset && !this.isRealDevice()) {
        ...
        let deviceArr = (await this.adjustedDeviceName()).split(" ");
        let deviceName;
        let deviceType = deviceArr[0] + " " + deviceArr[1];
        deviceName = deviceType;
        let sdkVersion = deviceArr[2].replace(/[()]/g, '');
        await this.createSim(deviceName, deviceType, sdkVersion);
