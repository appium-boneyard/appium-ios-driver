 ...
 async startSession () {
  ...
  async clearAppData  () {
    if (!this.keepAppToRetainPrefs && this.args.app && this.args.bundleId) {
      // TODO: this.sim.cleanCustomApp(path.basename(this.args.app), this.args.bundleId);
    }
  }
  ...
  async cleanupDeviceState () {
    ...
    if (this.realDevice && this.args.bundleId && this.args.fullReset) {
      ...
      try {
        // TODO: this.sim.cleanSim.bind(this.sim, this.args.keepKeyChains, this.args.tmpDir);
      } catch (err) {
