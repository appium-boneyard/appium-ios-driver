...
class IosDriver extends BaseDriver {
  ...
  async setLocale () {
    ...
    let setLoc = async () => {
      ...
      try {
        await this.sim.updateLocale(this.caps.language, this.caps.locale, this.caps.calendarFormat);
      } catch (e) {
    ...
    if ((this.caps.language || this.caps.locale || this.caps.calendarFormat) && (_.isNull(this.caps.udid) || _.isUndefined(this.caps.udid))) {
  ...
  setLocServicesPrefs () {
    let locServ = _.find([this.caps.locationServicesEnabled, this.caps.locationServicesAuthorized], (c) => {
    ...
    if (typeof this.caps.locationServicesAuthorized !== "undefined") {
      if (!this.caps.bundleId) {
        let msg = "Can't set location services for app without bundle ID";
        logger.error(msg);
        throw new Error(msg);
      }
      let locAuth = !!this.caps.locationServicesAuthorized;
      if (locAuth) {
  ...
  async navToInitialWebview () {
    ...
    } else if (parseInt(this.iosSdkVersion, 10) >= 7 && !this.caps.udid && this.caps.safari) {
    ...
    let spinHandles = async () => {
      ...
      if (!res) {
        if ((Date.now() - start) < 90000) {
          ...
          if (this.caps.udid || !this.caps.safari) {
