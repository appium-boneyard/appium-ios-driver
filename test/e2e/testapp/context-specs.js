...
describe('testapp - context methods', function () {
  ...
  it('getting list multiple times should not crash appium', async function () {
    for (let i = 0; i < 8; i++) {
  ...
  it('setting context to \'null\' should work', async function () {
    let contexts = await driver.getContexts();
    contexts.should.have.length(1);
  ...
  it('setting context to \'NATIVE_APP\' should work', async function () {
    let contexts = await driver.getContexts();
    contexts.should.have.length(1);
    await driver.setContext(contexts[0]);
  ...
  it('setting context to non-existent context should return \'NoSuchContext\' (status: 35)', async function () {
